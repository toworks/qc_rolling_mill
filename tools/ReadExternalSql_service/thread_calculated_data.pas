...
type
  ...
  TIdHeat = Record
    ...
    constructor Create(_tid: integer; _Heat, _Grade, _Section, _Standard, _StrengthClass,
                      ...
                      _LowRed, _HighRed, _LowGreen, _HighGreen: integer);
  end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      left.LowRed := Round(TempMax + AdjustmentMin) - 5;
      left.HighRed := Round(TempMin + AdjustmentMax) + 5;
      // -- report
...
{$ENDIF}
    ...
    begin
      // увеличиваем диапозон на 5 градусов
      right.LowRed := Round(TempMax + AdjustmentMin) - 5;
      right.HighRed := Round(TempMin + AdjustmentMax) + 5;
      // -- report
...
{$ENDIF}
  ...
  begin
    if InSide = 0 then
    begin
      left.LowGreen := Round(TempMax + AdjustmentMin);
      left.HighGreen := Round(TempMin + AdjustmentMax);
      LimitsM := 0;
      // -- report
...
{$ENDIF}
      ...
      right.LowGreen := Round(TempMax + AdjustmentMin);
      right.HighGreen := Round(TempMin + AdjustmentMax);
      LimitsM := 0;
      // -- report
...
initialization
left := TIdHeat.Create(0,'','','','','','','','','','','',0,false,0,0,0,0);
right := TIdHeat.Create(0,'','','','','','','','','','','',0,false,0,0,0,0);
