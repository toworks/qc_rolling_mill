...
uses
  SysUtils, Classes, Windows, ActiveX, System.Variants, Math, DBAccess, Ora,
...
type
  ...
  TIdHeat = Record
    ...
    constructor Create(_tid: integer; _Heat, _Grade, _Section, _Standard, _StrengthClass,
                      ...
                      _LowRed, _HighRed, _LowGreen, _HighGreen, _passing: integer);
  end;
...
//--      OraQuery.SQL.Add('''ETOPAHKXCBMetopahkxcbm'',''ЕТОРАНКХСВМеторанкхсвм'')');
      ...
      OraQuery.SQL.Add('''ETOPAHKXCBMetopahkxcbm'',''ЕТОРАНКХСВМеторанкхсвм'')');
      OraQuery.SQL.Add('or n.klass like translate('''+CutChar(StrengthClass)+''','); //переводим Eng буквы похожие на кирилицу
...
{$ENDIF}
    ...
    if (InSide = 0) and (left.passing <> 1) then
    begin
      // увеличиваем диапозон на 10 градусов
      left.LowRed := Round(TempMin + AdjustmentMin) - 10;
      left.HighRed := Round(TempMax + AdjustmentMax) + 10;
      // -- report
      CalculatedData(InSide, 'low_red=''' + inttostr(left.LowRed) + '''');
      CalculatedData(InSide, 'high_red=''' + inttostr(left.HighRed) + '''');
      // -- report
{$IFDEF DEBUG}
      SaveLog('debug' + #9#9 + 'LowRedLeft -> ' + inttostr(left.LowRed));
      SaveLog('debug' + #9#9 + 'HighRedLeft -> ' + inttostr(left.HighRed));
      SaveLog('debug' + #9#9 + 'left.passing first -> ' + inttostr(left.passing));
{$ENDIF}
      inc(left.passing); // маркер 2го прохода
    end
    else
    begin
      // увеличиваем диапозон на 5 градусов
      left.LowGreen := Round(TempMin + AdjustmentMin) - 5;
      left.HighGreen := Round(TempMax + AdjustmentMax) + 5;
      // -- report
      CalculatedData(InSide, 'low_green=''' + inttostr(left.LowGreen) + '''');
      CalculatedData(InSide, 'high_green=''' + inttostr(left.HighGreen) + '''');
      // -- report
{$IFDEF DEBUG}
      SaveLog('debug' + #9#9 + 'LowGreenLeft -> ' + inttostr(left.LowGreen));
      SaveLog('debug' + #9#9 + 'HighGreenLeft -> ' + inttostr(left.HighGreen));
      SaveLog('debug' + #9#9 + 'left.passing last -> ' + inttostr(left.passing));
{$ENDIF}
      left.passing := 0;
    end;

    if (InSide = 1) and (right.passing <> 1) then
    begin
      // увеличиваем диапозон на 10 градусов
      right.LowRed := Round(TempMin + AdjustmentMin) - 10;
      right.HighRed := Round(TempMax + AdjustmentMax) + 10;
      // -- report
      CalculatedData(InSide, 'low_red=''' + inttostr(right.LowRed) + '''');
      CalculatedData(InSide, 'high_red=''' + inttostr(right.HighRed) + '''');
      // -- report
{$IFDEF DEBUG}
      SaveLog('debug' + #9#9 + 'LowRedRight -> ' + inttostr(right.LowRed));
      SaveLog('debug' + #9#9 + 'HighRedRight -> ' + inttostr(right.HighRed));
      SaveLog('debug' + #9#9 + 'right.passing first -> ' + inttostr(right.passing));
{$ENDIF}
      inc(right.passing); // маркер 2го прохода
    end
    else
    begin
      // увеличиваем диапозон на 5 градусов
      right.LowGreen := Round(TempMin + AdjustmentMin) - 5;
      right.HighGreen := Round(TempMax + AdjustmentMax) + 5;
      // -- report
      CalculatedData(InSide, 'low_green=''' + inttostr(right.LowGreen) + '''');
      CalculatedData(InSide, 'high_green=''' + inttostr(right.HighGreen) + '''');
      // -- report
{$IFDEF DEBUG}
    SaveLog('debug' + #9#9 + 'LowGreenRight -> ' + inttostr(right.LowGreen));
    SaveLog('debug' + #9#9 + 'HighGreenRight -> ' + inttostr(right.HighGreen));
    SaveLog('debug' + #9#9 + 'right.passing last -> ' + inttostr(right.passing));
{$ENDIF}
      right.passing := 0;
    end;
