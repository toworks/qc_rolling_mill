...
//    DataModule1.OraQuery1.FetchAll := true;
    ...
    DataModule1.OraQuery1.SQL.Add('select * from czl_n where year=''2013''');
    DataModule1.OraQuery1.SQL.Add('where npl='+Heat+'');
    DataModule1.OraQuery1.Open;

      if count_chemical < DataModule1.OraQuery1.RecordCount then
       begin

        while not DataModule1.OraQuery1.Eof do
          begin
                heat := DataModule1.OraQuery1.FieldByName('fuse').AsString;
                grade := DataModule1.OraQuery1.FieldByName('mark').AsString;
                Ce := DataModule1.OraQuery1.FieldByName('C').AsFloat+
                      (DataModule1.OraQuery1.FieldByName('mn').AsFloat/6)+
                      (DataModule1.OraQuery1.FieldByName('cr').AsFloat/5)+
                      (DataModule1.OraQuery1.FieldByName('si').AsFloat/10);
                DateTime := DataModule1.OraQuery1.FieldByName('Date_Time').AsString;
...
//                sleep(500);
          ...
          count_chemical := DataModule1.OraQuery1.RecordCount;
  ...
  {$IFDEF DEBUG}
    SaveLog('debug'+#9#9+inttostr(DataModule1.OraQuery1.RecordCount));
  {$ENDIF}
