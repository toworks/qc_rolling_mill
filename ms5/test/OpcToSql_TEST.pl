...
=comm
	$query = "UPDATE temperature_current_ SET heat='$heat', ";
	...
	$query .= "IF \@\@ROWCOUNT=0 ";
	$query .= "INSERT INTO temperature_current_ (tid, [timestamp], ";
...
=cut
	$query = "UPDATE temperature_current_ SET heat=?, ";
	$query .= "grade=?, strength_class=?, ";
	$query .= "section=?, standard=?, ";
	$query .= "temperature=? where tid=? ";
	$query .= "and rolling_mill=? and side=? ";
	$query .= "IF \@\@ROWCOUNT=0 ";
	$query .= "INSERT INTO temperature_current_ (tid, [timestamp], ";
	$query .= "rolling_mill, heat, grade, strength_class, section, ";
	$query .= "standard, side, temperature) values ( ";
	$query .= "?, datediff(ss, '1970/01/01', GETDATE()), ";
	$query .= "?, ?, ?, ?, ";
	$query .= "?, ?, ?, ? )";
	...
	eval{ $sth = $self->{dbh}->prepare($query) || die $self->{log}->save(2, "Couldn't execute statement: " . $DBI::errstr);
		  $sth->bind_param( 1, "$heat" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 2, "N\'$grade\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 3, "N\'$StrengthClass\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 4, $section, SQL_DECIMAL ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 5, "N\'$standard\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 6, $temperature, SQL_DECIMAL ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 7, $tid ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 8, $rolling_mill ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 9, $side ) || die $self->{log}->save(2, $DBI::errstr);

		  $sth->bind_param( 10, $tid ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 11, $rolling_mill ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 12, "$heat" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 13, "N\'$grade\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 14, "N\'$StrengthClass\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 15, $section, SQL_DECIMAL ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 16, "N\'$standard\'" ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 17, $side ) || die $self->{log}->save(2, $DBI::errstr);
		  $sth->bind_param( 18, $temperature, SQL_DECIMAL ) || die $self->{log}->save(2, $DBI::errstr);
$self->{log}->save(4, "mssql query -> $query");
...
$log->save(4, "mssql dsn -> ".$mssql->get_dsn());
$log->save(4, "mssql get_username -> ".$mssql->get_username());
$log->save(4, "mssql get_password -> ".$mssql->get_password());
...
#			$log->save(4, "$tag => $items{$tag}->{timestamp}");
#			$log->save(4, "$tag => $items{$tag}->{value}");			
