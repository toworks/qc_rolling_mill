...
uses
  SysUtils, Classes, Variants, Math, ZDataset, Types{, Collections};
...
{$IFDEF DEBUG}
  SaveLog.Log(etDebug, 'enable left.marker -> '+booltostr(left.marker));
{$ENDIF}
      end;
...
{{$IFDEF DEBUG}
  SaveLog.Log(etDebug, 'RollingMill '+InRollingMill+' Side '+InSide+' left.marker -> '+booltostr(left.marker));
  SaveLog.Log(etDebug, 'RollingMill '+InRollingMill+' Side '+InSide+' right.marker -> '+booltostr(right.marker));
{$ENDIF}}
       SQueryRSOH.Next;
{$IFDEF DEBUG}
...
begin
  ...
  //если находим
  if OraQuery.FieldByName('NPL').IsNull then
  begin
