...
' ''ЕТОРАНКХСВМеторанкхсвм'',''ETOPAHKXCBMetopahkxcbm'')'+
...
exit;
{
  ...
  begin
    SaveLog.Log(etWarning, UTF8Encode('сторона -> '+side));
    SaveLog.Log(etWarning, UTF8Encode('недостаточно данных для расчета по плавкам -> '+HeatAll));
    exit;
  end;
...
{$ENDIF}
  ...
  begin
    SaveLog.Log(etWarning, UTF8Encode('сторона -> '+side));
    SaveLog.Log(etWarning, UTF8Encode('недостаточно данных по технологической инструкции для -> '+InHeat));
    exit;
  end;
...
{$ENDIF}
  ...
  begin
    SaveLog.Log(etWarning, UTF8Encode('сторона -> '+side));
    SaveLog.Log(etWarning, UTF8Encode('недостаточно данных по мех. испытаниям для расчета по плавкам -> '+HeatAll));
    exit;
  end;
...
{$ENDIF}
  ...
  begin
    SaveLog.Log(etWarning, UTF8Encode('сторона -> '+side));
    SaveLog.Log(etWarning, UTF8Encode('недостаточно данных по температуре для расчета по плавкам -> '+HeatAll));
    exit;
  end;
