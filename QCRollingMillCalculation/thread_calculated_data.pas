...
begin
  ...
  try
      ...
      OraQuery.SQL.Add(',n.razm1 section, REPLACE(n.klass, '' '', '''') strength_class');
...
//--      OraQuery.SQL.Add('''ETOPAHKXCBMetopahkxcbm'',''ЕТОРАНКХСВМеторанкхсвм'')');
      ...
      OraQuery.SQL.Add('and translate(REPLACE(n.klass, '' '', ''''),');
      OraQuery.SQL.Add('''ЕТОРАНКХСВМеторанкхсвм'',''ETOPAHKXCBMetopahkxcbm'')');
      OraQuery.SQL.Add('like translate('''+main.StrengthClass+''',');
      OraQuery.SQL.Add('''ЕТОРАНКХСВМеторанкхсвм'',''ETOPAHKXCBMetopahkxcbm'')');
...
{      if (main.StrengthClass = 'S400') or (main.StrengthClass = 'S400W') then // классы при которых берется до 15 проб
        OraQuery.SQL.Add('and nvl(n.npach,0)=nvl(v.npach,0) and NI<=15') NI - количество проб
      else
...
{$ENDIF}
    ...
    begin
      ...
      left.LowRed := Round(TempMin + AdjustmentMax - 0); //вместо 10
      left.HighRed := Round(TempMax + AdjustmentMin + 5); //вместо 10
      // -- report
...
{$ENDIF}
    ...
    begin
      ...
      left.LowGreen := Round(TempMin + AdjustmentMax - 0); //вместо 5
      left.HighGreen := Round(TempMax + AdjustmentMin + 2.5); //вместо 5
      // -- report
...
//      left.step := 0; // сброс этапа перенесен в начало
    ...
    begin
      // увеличиваем диапозон на 10 градусов
      right.LowRed := Round(TempMin + AdjustmentMax - 0); //вместо 10
      right.HighRed := Round(TempMax + AdjustmentMin + 5); //вместо 10
      // -- report
...
{$ENDIF}
    ...
    begin
      // увеличиваем диапозон на 5 градусов
      right.LowGreen := Round(TempMin + AdjustmentMax - 0); //вместо 5
      right.HighGreen := Round(TempMax + AdjustmentMin + 2.5); //вместо 5
      // -- report
